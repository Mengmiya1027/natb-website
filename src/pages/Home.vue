...
import { measureTextLayout } from '@/effects/masonTextLayout'
...
async function measureStaticTitle() {
  const host = particleRef.value
  if (!host) return
...
function baselineFix(layout, ink) {
  ...
  const target = stage.getBoundingClientRect().top + baselineCanvas / layout.ratio
...
function bootEffect() {
  ...
  if (!host || !host.clientWidth || !host.clientHeight) {
    rafId = requestAnimationFrame(bootEffect)
    return
  }
  effect = new TextToParticle(host, {
    text: TITLE,
...
function onResize() {
  ...
  resizeTimer = window.setTimeout(measureStaticTitle, 260)
}
...
<template>
  <div ref="stageRef" class="home">
    ...
    <h1
      class="home__title"
      :class="{ 'is-visible': staticVisible }"
      :style="[titleStyle, { transform: `translateX(${titleShift})` }]"
      aria-label="New Android Tool Box"
    >
      <span
      ...
      ><i v-if="index === 0" :ref="onProbe" class="home__probe"></i>{{ item.ch }}</span>
    </h1>
  </div>
</template>
